#include "signalflow/python/python.h"
    ...
    py::class_<AudioOut_Dummy, Node, NodeRefTemplate<AudioOut_Dummy>>(m, "AudioOut_Dummy", "Dummy audio output for offline processing")
        .def(py::init<int, int, int>(), "num_channels"_a = 2, "sample_rate"_a = SIGNALFLOW_DEFAULT_SAMPLE_RATE, "buffer_size"_a = SIGNALFLOW_DEFAULT_BLOCK_SIZE);
    ...
    py::class_<HistoryBufferWriter, Node, NodeRefTemplate<HistoryBufferWriter>>(m, "HistoryBufferWriter", "Writes a rolling history buffer of a given duration. At a given moment in time, the contents of the buffer will be equal to the past N seconds of the audio generated by `input`.")
        .def(py::init<BufferRef, NodeRef, int>(), "buffer"_a = nullptr, "input"_a = 0.0, "downsample"_a = 1);
    
    py::class_<SegmentPlayer, Node, NodeRefTemplate<SegmentPlayer>>(m, "SegmentPlayer", "Trigger segments of `buffer` at the given list of `onsets` positions, in seconds.")
    ...
    py::class_<Bus, VariableInputNode, NodeRefTemplate<Bus>>(m, "Bus", "Bus is a node with a fixed number of input channels and arbitrary number of inputs, used to aggregate multiple sources. It is similar to Sum, but with a defined channel count that does not adapt to its inputs.")
        .def(py::init<unsigned int>(), "num_channels"_a = 1);
    
    py::class_<ChannelArray, VariableInputNode, NodeRefTemplate<ChannelArray>>(m, "ChannelArray", "Takes an array of inputs and spreads them across multiple channels of output.")
        .def(py::init<>())
    ...
    py::class_<SelectInput, VariableInputNode, NodeRefTemplate<SelectInput>>(m, "SelectInput", "Pass through the output of one or more `inputs`, based on the integer input index specified in `index`. Unlike `ChannelSelect`, inputs may be multichannel, and `index` can be modulated in real time.")
    ...
    py::class_<Sum, VariableInputNode, NodeRefTemplate<Sum>>(m, "Sum", "Sums the output of all of the input nodes, by sample.")
        .def(py::init<>())
        .def(py::init<std::initializer_list<NodeRef >>(), "inputs"_a)
        .def(py::init<std::vector<NodeRef >>(), "inputs"_a)
        .def(py::init<std::vector<int >>(), "inputs"_a)
    ...
    py::class_<RandomBrownian, StochasticNode, NodeRefTemplate<RandomBrownian>>(m, "RandomBrownian", "Outputs Brownian noise between min/max, with a mean change of delta between samples. If a clock is passed, only generates a new value on a clock tick. To generate and hold an unchanging random value, pass clock=0.")
    ...
    py::class_<RandomChoice, StochasticNode, NodeRefTemplate<RandomChoice>>(m, "RandomChoice", "Pick a random value from the given array. If a clock is passed, only picks a new value on a clock tick. To generate and hold an unchanging random value, pass clock=0.")
        .def(py::init<std::vector<float >, NodeRef, NodeRef>(), "values"_a = std::vector<float> ( ), "clock"_a = nullptr, "reset"_a = nullptr);
    
    py::class_<RandomCoin, StochasticNode, NodeRefTemplate<RandomCoin>>(m, "RandomCoin", "Flip a coin with the given probability. If a clock is passed, only picks a new value on a clock tick. To generate and hold an unchanging random value, pass clock=0.")
        .def(py::init<NodeRef, NodeRef, NodeRef>(), "probability"_a = 0.5, "clock"_a = nullptr, "reset"_a = nullptr);
    
    py::class_<RandomExponentialDist, StochasticNode, NodeRefTemplate<RandomExponentialDist>>(m, "RandomExponentialDist", "Generate an random value following the exponential distribution. If a clock is passed, only picks a new value on a clock tick. To generate and hold an unchanging random value, pass clock=0.")
        .def(py::init<NodeRef, NodeRef, NodeRef>(), "scale"_a = 0.0, "clock"_a = nullptr, "reset"_a = nullptr);
    
    py::class_<RandomExponential, StochasticNode, NodeRefTemplate<RandomExponential>>(m, "RandomExponential", "Generate an random exponential value between min/max. If a clock is passed, only picks a new value on a clock tick. To generate and hold an unchanging random value, pass clock=0.")
        .def(py::init<NodeRef, NodeRef, NodeRef, NodeRef>(), "min"_a = 0.001, "max"_a = 1.0, "clock"_a = nullptr, "reset"_a = nullptr);
    
    py::class_<RandomGaussian, StochasticNode, NodeRefTemplate<RandomGaussian>>(m, "RandomGaussian", "Generate an random Gaussian value, with given mean and sigma. If a clock is passed, only picks a new value on a clock tick. To generate and hold an unchanging random value, pass clock=0.")
    ...
    py::class_<RandomUniform, StochasticNode, NodeRefTemplate<RandomUniform>>(m, "RandomUniform", "Generates a uniformly random value between min/max. If a clock is passed, only picks a new value on a clock tick. To generate and hold an unchanging random value, pass clock=0.")
